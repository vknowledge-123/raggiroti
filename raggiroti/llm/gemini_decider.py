...
class GeminiDecider:
    ...
    def decide(self, state: dict, retrieved: dict) -> dict:
        ...
        sys = (
            ...
            "Use state.liquidity_buy_levels / state.liquidity_sell_levels, swing highs/lows, and sweep/reclaim levels for SL and targets (liquidity pools). "
            "Use state.oi (walls + strike window) to prefer exits/targets into strong OI walls and to avoid entries into immediate resistance/support. "
            "Use state.recent_candles to detect momentum vs trap/chop (do not overtrade). "
